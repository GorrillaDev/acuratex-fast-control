...
static bool app_line_is_wifi_config_set(const char *line)
...

{
    ...
    return line != NULL && strncmp(line, "WIFI_CONFIG_SET", 15) == 0;
...
static const char *app_loggable_line(const char *line, char *scratch, size_t scratch_size)
...

{
    ...
    if (app_line_is_wifi_config_set(line))
...
static bool app_core0_service_command(const char *incoming_line)
...

{
    ...
    return app_file_transfer_is_command(line)



        || strcmp(line, "WIFI_CONFIG_GET") == 0



        || strncmp(line, "WIFI_CONFIG_SET", 15) == 0



        || strcmp(line, "WIFI_CONNECT") == 0;
...

{
...

    {
        ...
        ESP_LOGI(TAG, "[FAST] RX command=%s", app_loggable_line(line, scratch, sizeof(scratch)));
...

{
    ...
    char uart_log_line[96];



    ESP_LOGI(TAG, "RX UART rescate: %s", app_loggable_line(line, uart_log_line, sizeof(uart_log_line)));
...

{
    ...
    TickType_t usb_poll_delay = pdMS_TO_TICKS(10);
...

    {
...

        {
...

            {
                ...
                char usb_log_line[96];



                ESP_LOGI(TAG, "RX USB: %s", app_loggable_line(line, usb_log_line, sizeof(usb_log_line)));
        ...
        // Minimo un tick para evitar que usb_rx monopolice Core 0.



        vTaskDelay(usb_poll_delay);
...

{
...

    {
        ...
        ESP_LOGI(TAG,
                 ...
                 app_loggable_line(message.line, dispatch_log_line, sizeof(dispatch_log_line)));
        ...
        if (app_core0_service_command(message.line))
...

        {
            ...
            ESP_LOGI(TAG, "CMD CORE0_SERVICE|LINE=%s", app_loggable_line(message.line, dispatch_log_line, sizeof(dispatch_log_line)));



            esp_err_t local_err = app_command_process_line(message.line,
...

            {
                ...
                (void)app_reply_dispatcher_reply("ERR command process", &route);



                ESP_LOGW(TAG, "command_dispatch local fallo: %s", esp_err_to_name(local_err));
