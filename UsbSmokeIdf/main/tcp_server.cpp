...
static const char *app_tcp_loggable_line(const char *line, char *scratch, size_t scratch_size)
{
    if (line == NULL)
    {
        return "";
    ...
    if (strncmp(line, "WIFI_CONFIG_SET", 15) == 0)
    {
...
static void app_tcp_process_line(char *line, uint32_t session_id, int client_fd)
{
    ...
    char tcp_log_line[96];
    ESP_LOGI(TAG, "RX TCP: %s", app_tcp_loggable_line(line, tcp_log_line, sizeof(tcp_log_line)));
 ...
 *
 * [EQUIVALENCIA MCU]
 * Es una bandera de estado de un perif̩rico/logica de comunicacion.
 *
 * [SI NO EXISTIERA]
