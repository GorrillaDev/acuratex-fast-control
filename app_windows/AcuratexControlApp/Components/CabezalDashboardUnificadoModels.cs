// [ACURATEX] Modelos y protocolo del tablero de cabezal para sistema unificado.
...
///
/// [QUIEN LA USA]
/// La usa la vista unificada de cabezal.
///
/// [CUANDO SE USA]
...
/// </summary>
public sealed class CabezalMotorUnificado
{
    /// <summary>
    /// [POR QUE EXISTE]
    /// Este constructor existe para dejar el motor listo desde el inicio, sin pedir a la UI que arme cada campo manualmente.
    ///
    /// [QUIEN LO LLAMA]
    /// Lo llama el protocolo de la vista unificada cuando crea las listas de motores.
    ///
    /// [CUANDO SE EJECUTA]
    /// Se ejecuta durante la construccion del modelo, antes de cualquier interaccion.
    ///
    /// [ENTRADAS]
    /// No recibe parametros.
    ///
    /// [SALIDAS]
    /// Devuelve una instancia preparada para recibir valores por `init` y por propiedades editables.
    ///
    /// [EFECTOS SECUNDARIOS]
    /// No produce efectos externos.
    ///
    /// [FLUJO ACURATEX]
    /// Protocolo de cabezal -> new CabezalMotorUnificado -> UI.
    ///
    /// [EQUIVALENCIA MCU]
    /// Se parece a inicializar una estructura de control en RAM.
    ///
    /// [SI NO EXISTIERA]
    /// La clase dependeria de asignaciones separadas o podria quedar incompleta.
    /// </summary>
    public CabezalMotorUnificado()
    {
    }
    ...
    /// </summary>
    public IReadOnlyList<CabezalPositionUnificado> Positions { get; init; } = Array.Empty<CabezalPositionUnificado>();
    ...
    /// <summary>
    /// [POR QUE EXISTE]
    /// Esta propiedad existe para mostrar el texto exacto del comando asociado a este motor.
    ///
    /// [QUIEN LA USA]
    /// La usa la vista unificada cuando dibuja etiquetas o ayudas para el usuario.
    ///
    /// [CUANDO SE USA]
    /// Se calcula cada vez que la UI necesita renderizar el texto del comando.
    ///
    /// [ENTRADAS]
    ...
    ///
    /// [FLUJO ACURATEX]
    /// Modelo de motor -> `CommandLabel` -> pantalla.
    ///
    /// [EQUIVALENCIA MCU]
    /// Se parece a una etiqueta que resume un acceso a registro.
    ///
    /// [SI NO EXISTIERA]
    /// La vista tendria que repetir el mismo formateo en varios sitios.
    /// </summary>
    public string CommandLabel => $"320 1C {CabezalDashboardUnificadoProtocol.HexByte(MotorIndex)} LSB MSB";
}
...
///
/// [QUIEN LA USA]
/// La usa el motor unificado al mapear posiciones.
///
/// [CUANDO SE USA]
...
/// </summary>
public readonly record struct CabezalPositionUnificado(int Number, int Value);
...
///
/// [QUIEN LA USA]
/// La usa la pantalla de cabezal para los grupos J.
///
/// [CUANDO SE USA]
...
/// </summary>
public sealed class CabezalJGroupUnificado
{
    /// <summary>
    /// [POR QUE EXISTE]
    ...
    /// </summary>
    public CabezalJGroupUnificado(int number)
    {
        Number = number;
    }
...
///
/// [QUIEN LA USA]
/// La usa la vista del cabezal unificado.
///
/// [CUANDO SE USA]
...
/// </summary>
public sealed class CabezalOutputBlockUnificado
{
    /// <summary>
    /// [POR QUE EXISTE]
    ...
    ///
    /// [QUIEN LO LLAMA]
    /// Lo llama `CabezalDashboardUnificadoProtocol` al crear los bloques Yarn y Stitch.
    ///
    /// [CUANDO SE EJECUTA]
    ...
    /// </summary>
    public CabezalOutputBlockUnificado(
        string key,
        string title,
        string subtitle,
...
/// </summary>
public readonly record struct CabezalBlockPinChangeUnificado(string BlockKey, int PinIndex, bool On);
...
/// </summary>
public readonly record struct CabezalBlockAllChangeUnificado(string BlockKey, bool On);
...
///
/// [FLUJO ACURATEX]
/// Evento de protocolo -> `CabezalTraceEntryUnificado` -> consola o historial visual.
///
/// [EQUIVALENCIA MCU]
...
/// </summary>
public sealed record CabezalTraceEntryUnificado(DateTime Timestamp, string Direction, string Message);

/// <summary>
/// [POR QUÉ EXISTE]
/// Este helper existe para concentrar constantes, tablas y formateo del protocolo de cabezal.
///
/// [QUIEN LA USA]
/// La usa la vista unificada y sus servicios asociados.
///
/// [CUANDO SE USA]
...
/// </summary>
public static class CabezalDashboardUnificadoProtocol
{
    ...
    private static readonly CabezalPositionUnificado[] DenPositions =
    {
        new(1, 0x0000),
        new(2, 0x00A2),
        new(3, 0x0145),
    ...
    private static readonly CabezalPositionUnificado[] SicPositions =
    {
        new(1, 0x0000),
        new(2, 0x0176),
        new(3, 0x02EE),
    ...
    public static IReadOnlyList<CabezalMotorUnificado> CreateDenMotors()
    {
    ...
    public static IReadOnlyList<CabezalMotorUnificado> CreateSicMotors()
    {
    ...
    public static IReadOnlyList<CabezalOutputBlockUnificado> CreateYarnBlocks()
    {
        return new[]
        {
            new CabezalOutputBlockUnificado("yarn1", "Yarn 1", "320 1E 18..1F", new[] { 0x18, 0x19, 0x1A, 0x1B, 0x1C, 0x1D, 0x1E, 0x1F }, "y1_run", "y1_stop"),
            new CabezalOutputBlockUnificado("yarn2", "Yarn 2", "320 1E 24..27 / 20..23", new[] { 0x24, 0x25, 0x26, 0x27, 0x20, 0x21, 0x22, 0x23 }, "y2_run", "y2_stop"),
        };
    }

    public static IReadOnlyList<CabezalOutputBlockUnificado> CreateStitchBlocks()
    {
        return new[]
        {
            new CabezalOutputBlockUnificado("stitch1", "Stitch 1", "320 1E 00,01,02,05", new[] { 0x00, 0x01, 0x02, 0x05 }, "s_run_1", "s_stop_1"),
            new CabezalOutputBlockUnificado("stitch2", "Stitch 2", "320 1E 06,07,08,0B", new[] { 0x06, 0x07, 0x08, 0x0B }, "s_run_2", "s_stop_2"),
            new CabezalOutputBlockUnificado("stitch3", "Stitch 3", "320 1E 0C,0D,0E,11", new[] { 0x0C, 0x0D, 0x0E, 0x11 }, "s_run_3", "s_stop_3"),
            new CabezalOutputBlockUnificado("stitch4", "Stitch 4", "320 1E 12,13,14,17", new[] { 0x12, 0x13, 0x14, 0x17 }, "s_run_4", "s_stop_4"),
        };
    }
